...
```{r read}
# Reads in data
surveydata <- read.csv("data/Survey_results_final.csv", header = TRUE,
                  stringsAsFactors = F)
```
...
```{r}
# Prints out basic information on the data frame
names(surveydata)
str(surveydata)
head(surveydata)
tail(surveydata)
...
```{r}
# Selects and renames desired columns
survey <- surveydata %>% 
  ...
  rename(age = Answer.Age, gender = Answer.Gender, education = Answer.Education, income = Answer.HouseHoldIncome, sirius = Answer.Sirius.Radio,
...
```{r}
sum(is.na(survey)) # NA values in data frame

survey$age <- as.numeric(survey$age) # Converts age to a numeric value
survey %>% filter(is.na(age)) # Returns rows with NA for age
sum(is.na(survey)) # NA values in data frame
survey <- drop_na(survey) # Drops NA values introduced by converting age to numeric value

survey <- survey %>% filter(!age>115, !age<5) # Filters out unreasonable ages
# survey <- survey %>% filter(gender != "") # Filters out responses without a gender
survey <- survey %>% filter(!education == "select one") # Filters out responses without an education level
survey <- survey %>% filter(income != "") # Filters out responses without a selected income level
survey <- survey %>% filter(sirius != "") # Filters out responses without a response
```
...
ggplot(survey) + 
  geom_histogram(aes(x = as.numeric(age)), bins = 10, fill = "blue") +
...
survey %>%
  ...
  labs( title = "Gender Bar Chart", x = "Gender" , y = "Frequency")
survey %>% filter(gender != "Female", gender != "Male") # Rows without a response for gender
```
...
geom_bar(position = "stack") +
  labs( title = "Education Bar Chart", x = "Education Level" , y = "Frequency") +
...
survey %>%
  ggplot(aes(x = income)) + 
geom_bar(position = "stack") +
  labs( title = "Income Bar Chart", x = "Income" , y = "Frequency") +
theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
```

```{r}
# Listens to Sirius bar chart
survey %>%
  ggplot(aes(x = sirius)) + 
  geom_bar(position = "stack") +
  labs( title = "Listens to Sirius Chart", x = "Response" , y = "Frequency")
```

```{r}
# Listens to Wharton bar chart
survey %>%
  ggplot(aes(x = wharton)) + 
  geom_bar(position = "stack") +
  labs( title = "Listens to Wharton Station", x = "Response" , y = "Frequency")
```

```{r}
# Histogram of Work Time
ggplot(survey) + 
  geom_histogram(aes(x = worktime), bins = 10, fill = "blue") +
...
```{r}
df <- womendata %>% rename(Field = , Degree = Answer.Education, Sex = Answer.Gender, Year = , Number = )
```
